...
class CostTracker:
    ...
    def get_cost_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_cost_per_call': self.total_cost / sum(model['calls'] for model in self.calls_by_model.values()) if self.calls_by_model else 0
        }
...
class LLMMetricsTracker:
    ...
    def track_call(self, model: str, latency: float, success: bool = True, error_type: str = None):
...
class LangSmithManager:
    ...
    def __init__(self):
        ...
        if self.enabled and LANGSMITH_AVAILABLE:
            ...
            print("❌ LangSmith is enabled but not installed. Install with: pip install langsmith")
            self.enabled = False
    ...
    def trace_async_function(self, name: Optional[str] = None, **kwargs):
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapper(*args, **func_kwargs):
                trace_name = name or f"{func.__module__}.{func.__name__}"
                return await traceable(name=trace_name, **kwargs)(func)(*args, **func_kwargs)
    ...
    def _add_monitoring_wrapper(self, client):
        ...
        def monitored_chat_completions_create(*args, **kwargs):
            ...
            try:
                ...
                end_time = datetime.now()
                latency = (end_time - start_time).total_seconds()
                self.llm_metrics.track_call(model, latency, success=False, error_type=type(e).__name__)
                raise
    ...
    def _log_llm_call_with_costs(self, model: str, input_tokens: int, output_tokens: int, 
                                total_cost: float, latency: float, response):
        ...
        try:
            with trace(name=f"llm_call_{model}", run_type="llm") as run:
                ...
                run.outputs = {
                    "response": str(response.choices[0].message.content) if hasattr(response, 'choices') else str(response)
                }
                run.metadata = {
                    "model": model,
                    "input_tokens": input_tokens,
    ...
    def log_monitoring_summary(self):
        ...
        if cost_summary['calls_by_model']:
            ...
            for model, stats in cost_summary['calls_by_model'].items():
                print(f"  {model}: {stats['calls']} calls, ${stats['cost']:.4f}, {stats['tokens']:,} tokens")
    ...
    def log_agent_interaction(self, agent_name: str, user_message: str, agent_response: str, metadata: Optional[Dict] = None):
        """
...
def log_rag_operation(operation: str, query: str, result: Any, metadata: Optional[Dict] = None):
    """Convenience function for logging RAG operations."""
    manager = get_langsmith_manager()
    manager.log_rag_operation(operation, query, result, metadata)


def log_agent_interaction(agent_name: str, user_message: str, agent_response: str, metadata: Optional[Dict] = None):
...
def track_llm_call_manually(model: str, input_tokens: int, output_tokens: int, 
                           input_cost: float, output_cost: float, total_cost: float):
    """Manually track an LLM call for cases where automatic tracking isn't available."""
    manager = get_langsmith_manager()
    manager.cost_tracker.track_llm_call(model, input_tokens, output_tokens, 
                                       input_cost, output_cost, total_cost)


def track_llm_metrics_manually(model: str, latency: float, success: bool = True, error_type: str = None):
