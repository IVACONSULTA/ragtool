...
class TestRunner:
    ...
    def _run_security_test(self, test_path: Path, verbose: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(security_test_module, 'SecurityTester'):
                ...
                try:
                    # Test with mock URL
                    tester = security_test_module.SecurityTester("http://localhost:8001", "test-key")
                    
                    # Test that the class has required methods
                    required_methods = ['test_health_endpoint', 'test_authentication', 'test_input_validation']
    ...
    def _print_summary(self):
        ...
        for category, results in self.test_results.items():
            ...
            print(f"{category.replace('_', ' ').title():20} {status:8} "
                  f"({results['tests_passed']}/{results['tests_run']}) "
                  f"{results['execution_time']:.2f}s")
        
        print("-" * 50)
        print(f"{'TOTAL':20} {'✅ PASS' if all_categories_passed else '❌ FAIL':8} "
    ...
    def run_specific_tests(self, test_names: List[str], verbose: bool = True) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument('--category', choices=['unit_tests', 'integration_tests', 'security_tests', 'compliance_tests'], 
