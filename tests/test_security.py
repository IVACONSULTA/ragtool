...
class SecurityTester:
    ...
    def test_authentication(self) -> bool:
        ...
        if self.api_key:
            ...
            response = self.session.post(
                f"{self.base_url}/chat", json={"message": "test"}
            )
            if response.status_code == 200:
    ...
    def test_input_validation(self) -> bool:
        ...
        for test_name, malicious_input in test_cases:
            ...
            response = self.session.post(
                f"{self.base_url}/chat", json={"message": malicious_input}
            )
    ...
    def test_rate_limiting(self) -> bool:
        ...
        for i in range(105):  # Try to exceed typical rate limit
            response = self.session.post(
                f"{self.base_url}/chat", json={"message": f"test message {i}"}
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python test_security.py https://your-app.railway.app your_api_key"
        )
        sys.exit(1)
